...
export default function ReportIssueForm({
  ...
  React.useEffect(() => {
    ...
    recognition.onresult = async (event: any) => {
        ...
        // Only run analysis if the description field is empty
        if (form.getValues('description').trim() !== '') {
            toast({ title: "Heard You!", description: "Description field already has text. Appended your speech to it."});
        ...
        toast({ title: "Heard You!", description: "AI is analyzing your description..."});
        
        const result = await runIssueAnalysis(transcript, language);
        if (result.success && result.data) {
          ...
          form.setValue('description', transcript); // At least fill the description
        }
        setListeningState('idle');
    };
  }, [language, form, toast, listeningState]);
    ...
    <Card className="max-w-2xl mx-auto">
      ...
      <Form {...form}>
        <form ref={formRef} action={formAction} className="space-y-6">
          <CardContent className="space-y-6">
             {state && !state.success && (
              <Alert variant="destructive">
                ...
                <CardTitle>{t('Submission Failed')}</CardTitle>
          ...
          <CardFooter>
            <Button type="button" className="w-full" disabled={isCheckingDuplicates || isAnalyzing || listeningState !== 'idle'} onClick={handleSubmit}>
