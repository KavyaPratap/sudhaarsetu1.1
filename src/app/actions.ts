...
const issueSchema = z.object({
  ...
  userFrustration: z.coerce.number().min(1).max(5).optional().default(3),
  userUrgency: z.coerce.number().min(1).max(5).optional().default(3),
...
export async function submitIssue(
  ...
  const { title, description, category, photos, latitude, longitude, summary, userFrustration, userUrgency, peopleAffected, duration, userFeedback } = parseResult.data;
  ...
  try {
    ...
    const urgencyResult = await calculateUrgencyScore({
        ...
        downvotes: 0,
        userFrustration: userFrustration,
...
export async function runIssueAnalysis(
  ...
  try {
    const result = await analyzeIssue({ photoDataUri, ...feedback });
