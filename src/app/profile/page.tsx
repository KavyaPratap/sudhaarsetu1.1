...
import { Loader2, User as UserIcon, Award, Phone, MapPin, BadgeCheck, AlertCircle, LocateFixed } from 'lucide-react';
...
export default function ProfilePage() {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          if (addressTextareaRef.current) {
            ...
            // Manually trigger change for react-hook-form if needed
  ...
  React.useEffect(() => {
    ...
    if (state.success) {
      ...
      if (state.updatedProfile?.avatar_url) {
        setAvatarPreview(
          `${state.updatedProfile.avatar_url}?t=${new Date().getTime()}`
        );
      }
    } else if (state.error) {
       toast({
         ...
         <div className="flex min-h-screen items-center justify-center p-4">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>We couldn't load your profile. This can happen if the database security rules are not set up correctly.</CardDescription>
                </CardHeader>
                <CardFooter>
  ...
  const homeUrl = profile.role === 'admin' ? '/admin' : '/';
                ...
                <h3 className="text-lg font-semibold mb-4">{t('Your Stats')}</h3>
                 <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm">{profile?.contact_number ? `+91 ${profile.contact_number}` : t('No contact number')}</span>
                    </div>
                     <div className="flex items-center gap-3">
