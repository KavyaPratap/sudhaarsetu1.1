...
import type { Issue, WorkUpdate } from '@/lib/types';
...
async function getIssueDetails(issueId: string): Promise<FullIssue | null> {
  try {
    ...
    if (issueData.reportedBy) {
      ...
      if (uDoc.exists) reporterName = uDoc.data()?.full_name || 'Anonymous';
    }
...
export default async function MyWorkDetailPage({ params }: { params: Promise<{ id: string }> }) {
