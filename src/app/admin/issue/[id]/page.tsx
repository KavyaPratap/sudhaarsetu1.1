...
async function getIssueDetails(supabase: ReturnType<typeof createClient>, issueId: string): Promise<ExtendedIssue | null> {
    const { data: issueData, error: issueError } = await supabase
        .from('issues')
        .select(`
            *,
            assigned_worker:profiles!issues_assigned_worker_id_fkey(*),
            reporter:profiles!issues_reportedBy_fkey(*),
            quotes:quotes(*, profiles(full_name, avatar_url, average_rating)),
            ratings(*, profiles(full_name, role, avatar_url))
        `)
        .eq('id', issueId)
        .single();
    ...
    return issueData as ExtendedIssue;
}
